...
export default function ProgressScreen() {
  ...
  const getRadarScore = (type: string) => {
    ...
    if (typeGoals.length === 0) return Math.random() * 40 + 20;
  ...
  const getProgressFillStyle = (progress: number, status: string) => {
    const bgColor = status === "on_track" ? "#10B981" : status === "at_risk" ? "#F59E0B" : "#6366F1";
    return { height: 6, borderRadius: 3, width: progress + "%", backgroundColor: bgColor };
  };
  ...
  const getStatusBadgeStyle = (status: string) => {
    return {
      ...
      paddingVertical: 2,
      borderRadius: 6,
      backgroundColor: status === "on_track" ? "#10B98120" : "#F59E0B20"
    };
  };
  ...
  const getStatusTextStyle = (status: string) => {
    return {
      fontSize: 11,
      fontWeight: "500",
      color: status === "on_track" ? "#10B981" : "#F59E0B"
    };
  };

  const getInsightBorderColor = (type: string) => {
    return type === "win" ? "#10B981" : type === "risk" ? "#F59E0B" : type === "observation" ? "#6366F1" : "#9CA3AF";
  };
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
          ...
          {report.goalProgress.map((goal, idx) => (
            <View key={idx} style={styles.goalItem}>
              <View style={styles.goalHeader}>
                ...
                <Text style={styles.goalDelta}>{formatDelta(goal.progressDelta)}</Text>
              </View>
              <View style={styles.progressBar}>
                <View style={styles.progressFill as any} />
              </View>
              <View style={styles.goalMeta}>
                <Text style={styles.goalProgressText}>{goal.progressPct}%</Text>
                <View style={getStatusBadgeStyle(goal.status) as any}>
                  <Text style={getStatusTextStyle(goal.status) as any}>
                    {goal.status}
                  </Text>
                </View>
              </View>
        ...
        <View style={styles.section}>
          ...
          {report.insights.map((insight, idx) => (
            <View key={idx} style={[styles.insightCard, { borderLeftColor: getInsightBorderColor(insight.type) }]}>
          ...
          <View style={styles.section}>
            ...
            {report.recommendations.map((rec, idx) => (
              <View key={idx} style={styles.recCard}>
                ...
                <Text style={styles.recMeta}>{rec.resourceType} - {rec.estimatedTime}</Text>
              </View>
            ))}
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  radarContainer: { alignItems: "center", paddingVertical: 20 },
  radarChart: { width: 200, height: 200, position: "relative" },
  radarGrid: { position: "absolute", width: 160, height: 160, borderRadius: 80, borderWidth: 1, borderColor: "#374151", top: 20, left: 20 },
  radarGrid33: { width: 106, height: 106, borderRadius: 53, top: 47, left: 47 },
  radarGrid66: { width: 133, height: 133, borderRadius: 66, top: 33, left: 33 },
  radarCenter: { position: "absolute", width: 8, height: 8, borderRadius: 4, backgroundColor: "#6366F1", top: 96, left: 96 },
  radarAxis: { position: "absolute", width: 1, height: 80, backgroundColor: "#374151", top: 100, left: 100, transformOrigin: "top" },
  radarPoint: { position: "absolute", width: 12, height: 12, borderRadius: 6 },
  radarLabel: { position: "absolute", alignItems: "center" },
  radarLabelText: { fontSize: 12, color: "#9CA3AF", marginBottom: 2 },
  radarScore: { fontSize: 14, fontWeight: "600" },
  goalItem: { backgroundColor: "#1F2937", borderRadius: 12, padding: 16, marginBottom: 12 },
  goalHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 8 },
  goalTitle: { fontSize: 16, fontWeight: "600", color: "#FFFFFF", flex: 1 },
  goalDelta: { fontSize: 14, fontWeight: "600", color: "#10B981" },
  progressBar: { height: 6, backgroundColor: "#374151", borderRadius: 3, marginBottom: 8 },
  progressFill: { height: 6, borderRadius: 3 },
  goalMeta: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  goalProgressText: { fontSize: 12, color: "#9CA3AF" },
  statusBadge: { paddingHorizontal: 8, paddingVertical: 2, borderRadius: 6 },
  statusText: { fontSize: 11, fontWeight: "500" },
  insightCard: { flexDirection: "row", backgroundColor: "#1F2937", borderRadius: 12, padding: 16, marginBottom: 12, borderLeftWidth: 4, alignItems: "flex-start" },
  insightIcon: { fontSize: 20, marginRight: 12 },
  insightText: { flex: 1, fontSize: 15, color: "#E5E7EB", lineHeight: 22 },
  recCard: { backgroundColor: "#1F2937", borderRadius: 12, padding: 16, marginBottom: 12 },
  recTitle: { fontSize: 16, fontWeight: "600", color: "#FFFFFF", marginBottom: 4 },
  recDescription: { fontSize: 14, color: "#9CA3AF", marginBottom: 8 },
  recMeta: { fontSize: 12, color: "#6B7280" },
