...
export default function BigGoalScreen() {
  const router = useRouter();
  const [goal, setGoal] = useState("");

  const handleContinue = () => {
    if (goal.trim()) {
      router.push("/onboarding/lifetime-dream" as any);
    }
  };
  ...
  return (
    <LinearGradient colors={["#0D0D0D", "#111111"]} style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <Text style={styles.subtitle}>The one thing you want to achieve in the next 12 months</Text>
      ...
      <View style={styles.footer}>
        <TouchableOpacity
          style={[styles.continueButton, !goal.trim() && styles.continueButtonDisabled]}
...
const styles = StyleSheet.create({
  ...
  continueButtonDisabled: { backgroundColor: "#374151" },
